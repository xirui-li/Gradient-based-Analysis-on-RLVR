...
def organize_time_series(data):
    """Organize metrics by step → metric_name → component → layer → value"""
...
def plot_metric_by_layer(time_series_data, output_dir='plots_by_layer'):
    os.makedirs(output_dir, exist_ok=True)
    for metric_name, components in time_series_data.items():
        for component, layers in components.items():
            plt.figure(figsize=(12, 6))
            for layer, step_data in sorted(layers.items()):
                steps = sorted(step_data.keys())
                values = [step_data[step] for step in steps]
                plt.plot(steps, values, label=f'Layer {layer}')
            plt.xlabel("Step")
            plt.ylabel(metric_name)
            plt.title(f"{metric_name} over Steps | {component}")
            plt.legend()
            plt.grid(True, alpha=0.3)
            plt.tight_layout()
            fname = f"{metric_name}_{component.replace('.', '_')}_time_series.png"
...
def main():
    ...
    print("Plotting...")
    plot_metric_by_layer(time_series_data, output_dir="plots_by_layer")
    print("Done.")
