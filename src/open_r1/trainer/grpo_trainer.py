...
def recursive_convert(d):
    if isinstance(d, dict):
        ...
        return [safe_convert(v) for v in d]
    else:
        return safe_convert(d)

def append_grad_stats_to_jsonl(grad_stats: dict, filepath: str):
    grad_stats_serializable = recursive_convert(grad_stats)
    with open(filepath, "a") as f:
        f.write(json.dumps(grad_stats_serializable) + "\n")
...
class GRPOTrainer(Trainer):
    ...
    def _extract_global_gradients(self, accelerator, model):
        ...
        Filters out layer normalizations and embeddings to save memory.
        """
        ...
        # Patterns to skip
        skip_patterns = [
            'layernorm', 'layer_norm', 'ln_', '.norm',  # layer normalizations
            'embed', 'embedding', 'lm_head',            # embeddings and output layer
        ...
        for name, param in unwrapped_model.named_parameters():
            ...
            # Skip if name contains any of the skip patterns
        ...
        print("Finished extracting gradients!")
        return gradients
