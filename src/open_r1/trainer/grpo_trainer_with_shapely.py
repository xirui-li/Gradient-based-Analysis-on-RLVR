    ...
    def _inner_training_loop_with_shapley(
        ...
        if eval_dataloader is not None:
            ...
            if isinstance(eval_batch, dict):
                eval_batch = {k: v.to(args.device) for k, v in eval_batch.items()}
            else:
                eval_batch = eval_batch.to(args.device)

        # Check if evaluation dataset is bigger than training batch
        if eval_dataloader is not None and len(eval_dataloader) > self.args.train_batch_size:
            raise ValueError(
